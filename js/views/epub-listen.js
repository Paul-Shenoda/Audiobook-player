 ...
 * triggered them. An AI provider's *first* synthesis (loading the WASM
 * runtime, running inference) can take several seconds before it ever calls
 * `audio.play()` — by then Safari no longer honors the original tap, and
 * silently drops the call. The Web Speech fallback that follows a failed AI
 * provider is, by construction, *also* several async hops removed from that
 * same tap, so it hits the identical wall — the net effect is true silence,
 * with no error surfaced (WebKit doesn't fire `onerror` for this case
 * either). Playing an effectively-instant empty utterance and silent clip
 * synchronously, once per page session, "unlocks" both APIs for the rest of
 * the session so later async-triggered calls are still honored.
 */
let mediaUnlocked = false;
function unlockMediaForAutoplay() {
